(function(){
 ...
 function renderActivityTable(a){
  ...
  function normaliseEnablerOrg(raw){
    const v = clean(raw);
    if(!v) return [];
    const parts = v.split('/').map(x=>clean(x)).filter(Boolean);
    if(parts.length <= 1) return v ? [v] : [];
    if(parts[0].toUpperCase() === 'UNMISS' && parts.length >= 2){
      const primary = `UNMISS/${parts[1]}`;
      const rest = parts.slice(2).map(x=>x.toUpperCase() === 'UNMISS' ? 'UNMISS' : x);
  ...
  function enablerOrgList(r){
    return normaliseEnablerOrg(enablerOrgKey(r));
  }
  ...
  function enablerKpiCardsHTML(rows){
    ...
    const cards = [
      ...
      {icon:'🤝', label:'Reporting entities', value:orgs.length, sub:'Normalised count of reporting entities'},
