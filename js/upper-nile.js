(function(){
  ...
  const progressMethodologyText = 'Progress is calculated as the average of individual activity progress percentages, with each activity capped at 100%. Overall Activity Progress includes all planned activities, while Reported Activities Progress includes only activities with reported achievements.';
  const progressMethodologyTitle = progressMethodologyText.replace(/"/g, '&quot;');
  const progressInfoIcon = `<span class="un-progress-info-icon" title="${progressMethodologyTitle}" aria-label="Progress methodology">ⓘ</span>`;
  ...
  function objectiveComponentsHTML(components, code){
    ...
    const more = components.length > visibleCount
      ? `<button type="button" class="un-objective-more" data-expanded="false">+ ${components.length - visibleCount} more roadmap activit${components.length - visibleCount > 1 ? 'ies' : 'y'} ▾</button>`
      : '';
  ...
  function renderObjectives(a){
    const rows=D.objectives.map(o=>{
      ...
      const activityStats = activityProgressStats(roadmapActivitySummary(a, o.code));
      return {...o, components, overallProgress:activityStats.overall, reportedProgress:activityStats.reported};
    });
    byId('objective-cards').innerHTML=rows.map(r=>`
      <div class="un-objective-card un-objective-card-enhanced un-objective-card-components" data-objective="${r.code}">
        ...
        <div class="un-objective-stats un-objective-stats-progress">
          <span>Overall Activity Progress<br><strong>${r.overallProgress}%</strong></span>
          <span>Reported Activities Progress<br><strong>${r.reportedProgress}%</strong></span>
        </div>
      </div>`).join('');
 ...
 function renderActivityTable(a){
  ...
  function activityProgressStats(rows){
    // Method: calculate progress for each activity first, cap each activity at 100%,
    // then average activity percentages. This avoids adding mixed units such as
    // shelters, houses and square meters, while preventing overachievement from
    // inflating objective-level progress.
    const valid = (rows || [])
      .filter(r => (Number(r.target) || 0) > 0)
      .map(r => ({
        ...r,
        unitPct: clamp(pct(Number(r.achievement) || 0, Number(r.target) || 0))
      }));

    const reported = valid.filter(r => (Number(r.achievement) || 0) > 0);
    const avg = arr => arr.length ? Math.round(arr.reduce((sum,r)=>sum + r.unitPct, 0) / arr.length) : 0;
    ...
    return {
      ...
      notStartedCount: valid.filter(r => (Number(r.achievement) || 0) === 0).length,
      rows: valid,
      reportedRows: reported
    };
  }

  function activityOverallProgress(rows){
    return activityProgressStats(rows).overall;
  }

  function activityReportedProgress(rows){
    return activityProgressStats(rows).reported;
  }

  function objectiveInsightItems(g, rows, est, req, hasMixedUnits){
    const stats = activityProgressStats(rows);
    const activityProgress = stats.rows;
    const notStarted = stats.notStartedCount;
    const bestUnit = activityProgress
      .filter(r=>r.target>0 && r.achievement>0)
      .sort((a,b)=>b.unitPct-a.unitPct)[0];
    const items=[];
    items.push(`📊 Overall Activity Progress is <strong>${stats.overall}%</strong> across all planned roadmap activities.`);
    if(stats.reportedCount>0) items.push(`📈 Reported Activities Progress is <strong>${stats.reported}%</strong> across <strong>${stats.reportedCount}</strong> roadmap activit${stats.reportedCount>1?'ies':'y'} with reported achievements.`);
    ...
    if(notStarted>0) items.push(`⚠️ <strong>${notStarted}</strong> roadmap activit${notStarted>1?'ies':'y'} show zero reported unit progress.`);
    if(hasMixedUnits) items.push(`ℹ️ UNSRM progress is averaged from individual activity percentages, with each activity capped at <strong>100%</strong>, to avoid adding different units of measure.`);
  ...
  function roadmapProgressComparisonHTML(grouped){
    const rows = grouped.map(g => {
      const stats = activityProgressStats(g.rows);
      return {
        code: g.o.code,
        title: g.o.upperNileObjective,
        overall: stats.overall,
        reported: stats.reported,
        plannedCount: stats.plannedCount,
        reportedCount: stats.reportedCount,
    ...
    const lowest = [...rows].sort((a,b)=>a.overall-b.overall)[0];

    return `<div class="un-roadmap-progress-comparison" title="Progress is calculated as the average of individual roadmap activity percentages, with each activity capped at 100%. Overall Activity Progress includes all planned roadmap activities. Reported Activities Progress includes only activities with reported achievements.">
      <div class="un-roadmap-comparison-head">
        <div>
          <div class="un-kicker">UNSRM progress comparison</div>
          <h3>Progress by Upper Nile State Roadmap objective</h3>
          <p>Compares overall progress across all planned roadmap activities with progress among activities that have reported achievements.</p>
        </div>
      </div>
      <div class="un-roadmap-comparison-legend">
        <span><i class="overall"></i> Overall Activity Progress</span>
        <span><i class="reported"></i> Reported Activities Progress</span>
      </div>
      <div class="un-roadmap-comparison-bars">
        ${rows.map(r=>`
          <div class="un-roadmap-comparison-row" title="${r.code}: ${r.title.replace(/"/g,'&quot;')}\nOverall Activity Progress: ${r.overall}%\nReported Activities Progress: ${r.reported}%\nReported roadmap activities: ${r.reportedCount} of ${r.plannedCount}">
            <div class="un-roadmap-comparison-label">
              <strong>${r.code}</strong>
              <span>${r.plannedCount} roadmap activit${r.plannedCount === 1 ? 'y' : 'ies'}</span>
            </div>
            <div class="un-roadmap-comparison-barset">
              <div class="un-roadmap-comparison-bar overall"><em style="width:${clamp(r.overall)}%"></em><b>${r.overall}%</b></div>
              <div class="un-roadmap-comparison-bar reported"><em style="width:${clamp(r.reported)}%"></em><b>${r.reported}%</b></div>
            </div>
          </div>
        `).join('')}
      </div>
      <div class="un-roadmap-comparison-foot">
        <span>Highest overall progress: <strong>${best ? best.code : '—'}${best ? ` (${best.overall}%)` : ''}</strong></span>
        <span>Lowest overall progress: <strong>${lowest ? lowest.code : '—'}${lowest ? ` (${lowest.overall}%)` : ''}</strong></span>
      </div>
    </div>`;
  }
  ...
  function renderRoadmapSnapshot(grouped, a){
    ...
    const allActivityRows = grouped.flatMap(g=>g.rows);
    const overallStats = activityProgressStats(allActivityRows);
    const overallProgress = overallStats.overall;
    const reportedProgress = overallStats.reported;
    ...
    const objProgress = grouped.map(g=>({
      code:g.o.code,
      title:g.o.upperNileObjective,
      progress:activityOverallProgress(g.rows)
    })).sort((a,b)=>b.progress-a.progress);
    const bestProgress = objProgress[0];
    const lowestProgress = [...objProgress].sort((a,b)=>a.progress-b.progress)[0];
    const insightItems = [];
    if(bestProgress) insightItems.push(`🟢 Highest Overall Activity Progress: <strong>${bestProgress.code}</strong> (${bestProgress.progress}%).`);
    if(lowestProgress && lowestProgress !== bestProgress) insightItems.push(`🔴 Lowest Overall Activity Progress: <strong>${lowestProgress.code}</strong> (${lowestProgress.progress}%).`);
    if(overallStats.reportedCount>0) insightItems.push(`📈 Reported Activities Progress: <strong>${reportedProgress}%</strong> across <strong>${overallStats.reportedCount}</strong> roadmap activit${overallStats.reportedCount>1?'ies':'y'} with reported achievements.`);
    if(notStarted>0) insightItems.push(`⚠️ <strong>${notStarted}</strong> roadmap activit${notStarted>1?'ies':'y'} have zero reported unit progress.`);
    insightItems.push(`📍 Roadmap coverage spans <strong>${counties}</strong> count${counties===1?'y':'ies'} under the selected filters.`);
    ...
    return `<div class="un-roadmap-snapshot">
      ...
      <div class="un-roadmap-snapshot-kpis">
        ...
        <div><strong>${activities}</strong><span>Roadmap Activities</span></div>
        <div title="${progressMethodologyTitle}"><strong>${overallProgress}%</strong><span>Overall Activity Progress ${progressInfoIcon}</span></div>
        <div><strong>${counties}</strong><span>Counties</span></div>
      </div>
  ...
  function roadmapTabbedTablesHTML(a, objectiveCode, rows, headers, totalRow){
    ...
    return `<details class="un-roadmap-details">
  ...
  function renderRoadmap(a){
    ...
    const cards = grouped.map(g=>{
      ...
      const progressStats = activityProgressStats(g.rows);
      const overallProgress = progressStats.overall;
      const reportedProgress = progressStats.reported;
      ...
      const totalRow=`<tr class="un-total-row"><td>Total / average</td><td></td><td></td><td>${hasMixedUnits ? '<span class="un-total-dash">—</span>' : num(av)}</td><td>${hasMixedUnits ? '<span class="un-total-dash">—</span>' : num(tv)}</td><td>${money(est)}</td><td>${money(req)}</td><td>${progressBarHTML(overallProgress)}</td><td>${progressBarHTML(fp)}</td><td><span class="${fundingGapClass(req-est)}">${fmtCompact(req-est, true)}</span></td></tr>`;
      ...
      return `<div class="un-roadmap-item un-roadmap-story-card">
        <div class="un-roadmap-story-grid">
          <aside class="un-roadmap-visual-panel">
            ...
            <div class="un-roadmap-big-number">${overallProgress}%</div>
            <div class="un-roadmap-big-label" title="${progressMethodologyTitle}">Overall Activity Progress ${progressInfoIcon}</div>
            <div class="un-roadmap-single-progress"><span style="width:${clamp(overallProgress)}%"></span></div>
            <div class="un-roadmap-reported-progress">
              <span title="${progressMethodologyTitle}">Reported Activities Progress ${progressInfoIcon}</span>
              <strong>${reportedProgress}%</strong>
              <div><em style="width:${clamp(reportedProgress)}%"></em></div>
            </div>
    ...
    byId('roadmap-objective-detail').innerHTML = renderRoadmapSnapshot(grouped, a) + roadmapProgressComparisonHTML(grouped) + cards;
  ...
  function renderSelectedCountySnapshot(a){
    ...
    el.innerHTML = `
      <div class="un-county-snapshot-section un-county-selection-block">
        ...
        <p>${progressReportedActivities} of ${plannedActivities} planned activities have reported achievements.</p>
      </div>
  ...
  function countyActivityStatus(r){
    return (Number(r.achievement) || 0) > 0 ? 'Reported' : 'Planned / not yet reported';
  }
  ...
  function renderCountyActivityExplorer(rowsForTable){
    ...
    explorer.innerHTML = `
      ...
      <label class="un-county-table-search">
        ...
        <div>
          <input id="county-activity-search" type="search" placeholder="Search county, year, activity, unit, or organisation..." autocomplete="off">
    ...
    function rowsForTab(tab){
      if(tab === 'planned'){
        return rowsForTable
          ...
          .sort((a,b)=> String(a.county).localeCompare(String(b.county), undefined, {numeric:true}) || String(a.activity).localeCompare(String(b.activity), undefined, {numeric:true}));
      }
      return rowsForTable
        .filter(r => r.reportingStatus === 'Reported')
        .sort((a,b)=> String(a.county).localeCompare(String(b.county), undefined, {numeric:true}) || String(a.activity).localeCompare(String(b.activity), undefined, {numeric:true}));
    ...
    function activityCards(county, mode){
      ...
      return activityRows.map(act=>{
        ...
        const mini = mode === 'pending'
          ? `<div class="un-county-coverage-pending-note">No achievement reported</div>`
    ...
    function listForMode(mode){
      ...
      return rows.map(r=>{
        ...
        const title = mode === 'pending'
          ...
          : `Activity reporting coverage: ${r.reportedCount} of ${r.plannedCount} planned activities have reported achievements`;
    ...
    return `<div class="un-county-coverage-visual">
      <div class="un-county-coverage-header">
        <div>
          ...
          <p>The bars compare activities with reported achievements against total planned activities. Use the tabs to see reported activities or planned activities without reported progress. Activity names are shown exactly as reported in the roadmap data.</p>
        </div>
