(function(){
  ...
  const roadmapObjectiveLabel = code => {
    const n = codeNumber(code);
    return Number.isFinite(n) && n !== 999 ? `Upper Nile State Roadmap Objective ${n} (${code})` : code;
  };
  ...
  function renderKpis(a){
    const hlp=sumAchievementBy('UNSRM 2.1.1');
    const houses=sumAchievementBy('UNSRM 1.1.1');
    const livelihoodTraining=sumAchievementBy('UNSRM 3.2');
    const livelihoodSupport=sumAchievementBy('UNSRM 3.3');
    const hospitals=sumAchievementBy(['UNSRM 4.2.2']);
    const schools=sumAchievementBy('UNSRM 4.3');
    const waterPlants=sumAchievementBy('UNSRM 4.1.2');
    const peaceStructures=sumAchievementBy('UNSRM 5.1.2');
    const kpis=[
      ['Funding Needed', fmtCompact(a.requirement,true), 'Roadmap funding requirement'],
      ['Estimated Funding Invested', fmtCompact(a.estimated,true), 'Reported estimated funding'],
      ...(state.objective !== 'All' ? [['% Progress Financial', pct(a.estimated,a.requirement)+'%', 'Estimated funding / requirement']] : []),
      ['HHs reached with HLP', fmtCompact(hlp), 'UNSRM 2.1.1'],
      ['Houses constructed', fmtCompact(houses), 'UNSRM 1.1.1'],
      ['HHs supported for livelihood', fmtCompact(livelihoodSupport), 'UNSRM 3.3'],
      ['HHs trained / livelihood', fmtCompact(livelihoodTraining), 'UNSRM 3.2'],
      ['Hospitals supported', fmtCompact(hospitals), 'UNSRM 4.2.2'],
      ['Schools supported', fmtCompact(schools), 'UNSRM 4.3'],
      ['Water plants rehabilitated', fmtCompact(waterPlants), 'UNSRM 4.1.2'],
      ['Peace structures supported', fmtCompact(peaceStructures), 'UNSRM 5.1.2']
    ];
    byId('kpi-grid').classList.add('powerbi-kpis');
    byId('kpi-grid').innerHTML=kpis.map(k=>`<div class="un-kpi"><div class="un-kpi-label">${k[0]}</div><div class="un-kpi-value">${k[1]}</div><div class="un-kpi-sub">${k[2]}</div></div>`).join('');
  }
  ...
  function renderObjectives(a){
    const rows=D.objectives.map(o=>{
      const ach=a.achievements.filter(r=>objKey(r)===o.code);
      const tar=a.targets.filter(r=>objKey(r)===o.code);
      const av=ach.reduce((s,r)=>s+progressUnits(r),0);
      const tv=targetUnitCalculation(tar);
      const est=ach.reduce((s,r)=>s+progressFinancial(r),0);
      const req=targetFinancialCalculation(tar);
      return {...o, av,tv,est,req, progress:pct(av,tv), activityCount:unique(tar.map(r=>clean(r['UNS Activity Indicator Code']))).length};
    });
    byId('objective-cards').innerHTML=rows.map(r=>`<div class="un-objective-card" data-objective="${r.code}"><div class="un-objective-code">${roadmapObjectiveLabel(r.code)}</div><h4>${r.upperNileObjective}</h4><div class="un-progress"><span style="width:${clamp(r.progress)}%"></span></div><div class="un-objective-stats"><span>Progress<br><strong>${r.progress}%</strong></span><span>Activities<br><strong>${r.activityCount}</strong></span><span>Funding<br><strong>${money(r.est)}</strong></span><span>Gap<br><strong class="${r.req-r.est>0?'fund-gap':'fund-pos'}">${money(r.req-r.est)}</strong></span></div></div>`).join('');
 ...
 function renderActivityTable(a){
  ...
  function renderSelectedCountySnapshot(a){
    ...
    el.innerHTML = `
      ...
      <div class="un-county-objectives-box">
        <div class="un-county-box-title">UNSRM Objectives Active</div>
  ...
  function renderSelectedPartnerSnapshot(rows){
    ...
    el.innerHTML = `
      ...
      <div class="un-mini-metric"><strong>${objectives.length}</strong><span>UNSRM objectives covered</span></div>

      <div class="un-county-objectives-box">
        <div class="un-county-box-title">UNSRM Objectives Covered</div>
        <div class="un-county-objective-pills">
  ...
  function renderSelectedEnablerSnapshot(rows){
    ...
    el.innerHTML = `
      ...
      <div class="un-county-objectives-box">
        <div class="un-county-box-title">UNSRM Objectives</div>
        <div class="un-county-objective-pills">
          ${objectives.length ? objectives.map(o=>`<span>${o}</span>`).join('') : '<em>Not reported</em>'}
        </div>
      </div>
  ...
  function countyFillColor(reportingCoverage, plannedActivities){
    // County map shading is based on reporting coverage:
    // reported activities ÷ planned activities.
    if(!plannedActivities || plannedActivities <= 0) return '#E5E7EB';
    if(reportingCoverage <= 0) return '#E5E7EB';
    if(reportingCoverage <= 25) return '#bfe8f7';
    if(reportingCoverage <= 50) return '#5bb4dc';
    if(reportingCoverage <= 75) return '#137fb5';
    return '#003f63';
  }
  ...
  function updateMap(){
    ...
    svgEntries.forEach(entry=>{
      ...
      .attr('fill',d=>{
        ...
        return countyFillColor(m.reportingCoverage, m.plannedActivities);
      })
      .attr('stroke',d=>selected===d.properties.ADM2_EN?'#042f46':'#ffffff')
      .attr('stroke-width',d=>selected===d.properties.ADM2_EN?2.8:1.1)
      ...
      .on('mousemove',function(event,d){
        ...
        if(entry.id === 'upper-nile-enabler-map'){
          ...
          tooltip.innerHTML=`<strong>${name}</strong><br>Progress reported: <b>${m.reportedActivities} of ${m.plannedActivities} activities</b><br>Reporting organisations: <b>${m.reportingOrgs.length}</b><br>Implementing organisations: <b>${m.implementingOrgs ? m.implementingOrgs.length : 0}</b><br>Estimated funding invested: <b>${fmtCompact(m.estimated,true)}</b><br>Funding progress: <b>${m.financialProgress}%</b><br>Funding gap: <b>${fmtCompact(m.gap,true)}</b><br><span class="un-tooltip-hint">Click to filter</span>`;
        }
      })
  ...
  function renderAll(){
    const a=aggregate(); renderKpis(a); renderFinancialProgressMatrix(a); renderAlignment(); renderObjectives(a); renderActivityTable(a); renderRoadmap(a); renderCounty(a); renderPartners(a); renderEnablers(a); renderInsights(a); renderPartnerSnapshot(a); updateMap();
  }
